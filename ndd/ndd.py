...
def main():
  ...
  filenames_of_first_ten = [
      detector.filename("file%02d.txt") % (f,) for f in range(10)
  ]
  filenames_of_first_one_hundred = [
      detector.filename("file%02d.txt") % (f,) for f in range(100)
  ]
  for index1, j in enumerate(filenames_of_first_ten):
    ...
    for near in sorted(three_nearest, key=operator.itemgetter(1), reverse=True):
      print(
          "\t%s with Jaccard coefficient of %0.3f"
          % ("file%02d.txt" % near[0], near[1])
      )
